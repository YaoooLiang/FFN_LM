...
class Canvas(object):
    ...
    def predict(self, pos):
        ...
        #startpos = (346,346,374)
        #start_s = np.array(startpos) - self.margin
        #end_s = start_s + self.input_size
        assert np.all(start >= 0)
        ...
        #seeds_s = self.seed[start_s[0]:end_s[0], start_s[1]:end_s[1], start_s[2]:end_s[2]].copy()
        init_prediction = np.isnan(seeds)
        seeds[init_prediction] = np.float32(logit(0.05))
        #init_prediction = np.isnan(seeds_s)
        #seeds_s[init_prediction] = np.float32(logit(0.05))

        images = torch.from_numpy(images).float().unsqueeze(0)
        seeds = torch.from_numpy(seeds).float().unsqueeze(0).unsqueeze(0)
        #seeds_s = torch.from_numpy(seeds_s).float().unsqueeze(0).unsqueeze(0)

        #slice = seeds[:, :, seeds.shape[2] // 2, :, :].sigmoid()
        #seeds[:, :, seeds.shape[2] // 2, :, :] = slice
        #slice_s = seeds_s[:, :, seeds_s.shape[2] // 2, :, :].sigmoid()
        #seeds_s[:, :, seeds_s.shape[2] // 2, :, :] = slice_s
    ...
    def segment_at(self, start_pos):
        ...
        for pos in self.movement_policy:
            ...
            stacked_img = np.stack((self.seg_prob_i,) * 3, axis=-1)
            stacked_img[tuple(sel_cent)] = (250,0,0)

            skimage.io.imsave('./data/sample_{}.tif'.format(num_iters),stacked_img)
